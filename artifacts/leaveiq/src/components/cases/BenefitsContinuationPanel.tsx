...
export function BenefitsContinuationPanel({ caseId, employeeEmail }: Props) {
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(135deg, #F0F7F4 0%, #E8F5F0 100%)", borderColor: "#D9B381" }}
    >
      {/* Header */}
      <button
        className="w-full flex items-center justify-between px-5 py-4 text-left"
        style={{ background: "linear-gradient(135deg, #B68B5E 0%, #6B5230 100%)" }}
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-semibold" style={{ color: "#5E4A2E" }}>
                Continuing Benefits
              </p>
              <p className="text-xs font-medium" style={{ color: "#B68B5E" }}>
              ...
              {benefits.map((b) => (
                <div key={b.id} className="flex gap-2 items-center">
                  <input
                    ...
                    style={{ borderColor: "#D9B381", color: "#5E4A2E", background: "#FCFBF7" }}
                  />
                  <div className="flex items-center gap-1">
                    <span className="text-sm" style={{ color: "#C6A074" }}>$</span>
                    <input
                      type="number"
                      ...
                      style={{ borderColor: "#D9B381", color: "#5E4A2E", background: "#FCFBF7" }}
                    />
                    <span className="text-xs" style={{ color: "#C6A074" }}>/mo</span>
                  </div>
                  <button
            ...
            <div className="flex gap-2 mt-3">
              <button
                ...
                style={{ borderColor: "#D9B381", color: "#B68B5E", background: "#FCFBF7" }}
              >
              ...
              {COMMON_BENEFITS.filter(
                ...
                <button
                  ...
                  style={{ borderColor: "#D9B381", color: "#C6A074", background: "#FCFBF7" }}
                >
                  + {name.split(" ")[0]}
                </button>
              ))}
          ...
          {generating && (
            <div className="flex items-center justify-center gap-2 py-4">
              <Loader2 className="w-5 h-5 animate-spin" style={{ color: "#B68B5E" }} />
              <p className="text-sm" style={{ color: "#B68B5E" }}>Generating letter…</p>
            </div>
          )}
          ...
          {letter && !sendSuccess && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold flex items-center gap-1.5" style={{ color: "#5E4A2E" }}>
                  <Edit3 className="w-3.5 h-3.5" /> Generated Letter — Review &amp; Edit
                </p>
                <button
                  onClick={generate}
                  className="text-xs flex items-center gap-1" style={{ color: "#C6A074" }}
                >
              ...
              {/* Email + send */}
              <div>
                <label className="text-xs font-semibold block mb-1" style={{ color: "#5E4A2E" }}>
                  Employee Email
                </label>
                <div className="flex items-center gap-2">
                  <Mail className="w-4 h-4 shrink-0" style={{ color: "#C6A074" }} />
                  <input
                    type="email"
                    value={overrideEmail}
