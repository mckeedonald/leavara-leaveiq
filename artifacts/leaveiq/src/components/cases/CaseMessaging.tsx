  ...
  return (
    <div className="flex flex-col" style={{ minHeight: "300px" }}>
      ...
      >
        ...
        <span className="font-semibold text-sm" style={{ color: "#1B2430" }}>Case Messages</span>
        <span className="ml-auto text-xs" style={{ color: "#5A6470" }}>
          {messages.length} {messages.length === 1 ? "message" : "messages"} · saved to case file
        </span>
      </div>

      {/* Message list */}
      <div ref={listRef} className="flex-1 overflow-y-auto p-4 space-y-3" style={{ maxHeight: "380px", background: "#FAF9F4" }}>
        {loading ? (
          <div className="text-center py-8 text-xs" style={{ color: "#5A6470" }}>Loading messages…</div>
        ) : messages.length === 0 ? (
          <div className="text-center py-8">
            <MessageSquare className="w-8 h-8 mx-auto mb-2" style={{ color: "#D0C7B2" }} />
            <p className="text-xs" style={{ color: "#5A6470" }}>
                ...
                <div
                  ...
                  style={
                    mine
                      ...
                      : { background: "#FFF", color: "#1B2430", border: `1px solid ${borderColor}`, borderRadius: "18px 18px 18px 4px" }
                  }
                >
                  {!mine && (
                  ...
                  <p
                    ...
                    style={{ color: mine ? "rgba(255,255,255,0.7)" : "#5A6470" }}
                  >
      ...
      <div className="border-t p-3 flex gap-2" style={{ borderColor, background: cardBg }}>
        <textarea
          ...
          style={{ borderColor, color: "#1B2430", background: "#FAF9F4" }}
        />
        <button
          onClick={handleSend}
