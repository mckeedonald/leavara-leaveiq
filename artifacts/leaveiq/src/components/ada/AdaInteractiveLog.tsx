...
const ENTRY_CONFIG: Record<string, {
  ...
  physician_cert_sent:   { icon: <Stethoscope className="w-4 h-4" />,   label: "Physician Cert Sent",  bg: "#FBF7EF", text: "#8E6A45", border: "#EDE2CE" },
  ...
  jan_lookup:            { icon: <ClipboardList className="w-4 h-4" />, label: "JAN Research",         bg: "#FBF7EF", text: "#8E6A45", border: "#EDE2CE" },
  ada_determination:     { icon: <AlertCircle className="w-4 h-4" />,   label: "ADA Determination",    bg: "#FEF3C7", text: "#92400E", border: "#FDE68A" },
  ada_agent:             { icon: <Bot className="w-4 h-4" />,           label: "Ada Agent",            bg: "#FBF7EF", text: "#8E6A45", border: "#EDE2CE" },
};
    ...
    <div>
      {!open ? (
        ...
        <form
          ...
          style={{ borderColor: "#EDE2CE", background: "#FBFAF5" }}
        >
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="text-xs font-medium block mb-1" style={{ color: "#8E6A45" }}>Entry Type</label>
              <select
                value={entryType}
          ...
          <div>
            <label className="text-xs font-medium block mb-1" style={{ color: "#8E6A45" }}>Notes</label>
            <textarea
              value={content}
...
export function AdaInteractiveLog({ entries, caseId, onRefresh }: AdaInteractiveLogProps) {
  ...
  return (
    <div className="rounded-2xl border overflow-hidden" style={{ borderColor: "#EDE2CE" }}>
      {/* Header */}
      <button
        onClick={() => setCollapsed((c) => !c)}
        className="w-full flex items-center justify-between px-5 py-4 text-left transition-colors hover:bg-violet-50/50"
        style={{ background: "#FBF7EF", borderBottom: collapsed ? "none" : "1px solid #EDE2CE" }}
      >
        <div className="flex items-center gap-2">
          <ClipboardList className="w-4 h-4" style={{ color: "#B68B5E" }} />
          <span className="font-semibold text-sm" style={{ color: "#6B5230" }}>
            Interactive Process Log
          </span>
          <span
            className="ml-2 px-2 py-0.5 rounded-full text-xs font-bold"
            style={{ background: "#EDE2CE", color: "#8E6A45" }}
          >
            {entries.length}
          </span>
        </div>
      ...
      {!collapsed && (
        <div className="p-5 flex flex-col gap-4" style={{ background: "#FBFAF5" }}>
          {entries.length === 0 ? (
            <div className="text-center py-8 text-sm" style={{ color: "#8E6A45" }}>
