...
/** Build an ADA-specific PDF using the shared generator with a custom title. */
function buildAdaPdf(content: string): Buffer {
  return buildMedCertPdf(content, "ADA PHYSICIAN CERTIFICATION REQUEST");
}
...
router.post("/ada/cases/:caseId/physician-cert", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Generate PDF using the shared builder (ADA title, same algorithm as FMLA med cert)
    const pdfBuffer = buildAdaPdf(certWithInstructions);
